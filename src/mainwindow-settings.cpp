...
void MainWindow::readSettings()
{
    ...
    //General
    strcpy(settings_general_language, qPrintable(settings.value("Language", "default").toString()));
    strcpy(settings_general_icon_theme, qPrintable(settings.value("IconTheme", "default").toString()));
    ...
    strcpy(settings_general_mdi_bg_logo, qPrintable(settings.value("MdiBGLogo", appDir + "/images/logo-spirals.png").toString()));
    strcpy(settings_general_mdi_bg_texture, qPrintable(settings.value("MdiBGTexture", appDir + "/images/texture-spirals.png").toString()));
    ...
    strcpy(settings_display_units, qPrintable(settings.value("Display/Units", "mm").toString()));
    ...
    strcpy(settings_prompt_font_family,
        qPrintable(settings.value("Prompt/FontFamily", "Monospace").toString()));
    strcpy(settings_prompt_font_style,
    ...
    strcpy(settings_prompt_save_history_filename, qPrintable(settings.value("Prompt/SaveHistoryFilename", settingsDir + "prompt.log").toString()));

    //OpenSave
    strcpy(settings_opensave_custom_filter,
        qPrintable(settings.value("OpenSave/CustomFilter", "supported").toString()));
    strcpy(settings_opensave_open_format,
    ...
    strcpy(settings_opensave_save_format,
    ...
    for (i=0; i<MAX_FILES-1; i++) {
        strncpy(settings_opensave_recent_list_of_files[i], qPrintable(sl[i]),
            MAX_STRING_LENGTH);
    }
    strncpy(settings_opensave_recent_list_of_files[i], "END", MAX_STRING_LENGTH);
    strncpy(settings_opensave_recent_directory,
        qPrintable(settings.value("OpenSave/RecentDirectory", appDir + "/samples").toString()),
        MAX_STRING_LENGTH);
    */
    strncpy(settings_opensave_recent_list_of_files[0], "END", MAX_STRING_LENGTH);

    //Trimming
    settings_opensave_trim_dst_num_jumps = settings.value("OpenSave/TrimDstNumJumps", 5).toInt();

    //Printing
    strcpy(settings_printing_default_device, qPrintable(settings.value("Printing/DefaultDevice", "").toString()));
    ...
    strcpy(settings_grid_type, qPrintable(settings.value("Grid/Type", "Rectangular").toString()));
    ...
    //Text
    strcpy(settings_text_font, qPrintable(settings.value("Text/Font", "Arial").toString()));
...
void MainWindow::writeSettings()
{
    ...
    for (int i=0; i<MAX_FILES; i++) {
        ...
        sl << QString(settings_opensave_recent_list_of_files[i]);
    }
