...
QString settingsDir = QDir::homePath() + "/.embroidermodder2/";
#else
QString settingsDir = "";
#endif
std::string defaultsPath = "./defaults.toml";
QString settingsPath = settingsDir + "settings.toml";
...
void store_string(std::string key, QString value)
{
...
QString str_from_toml(toml::value value)
{
    return QString::fromLocal8Bit(toml::get<std::string>(value));
}
...
void MainWindow::load_configuration(void)
{
    ...
    for (const auto &[i, j] : config.as_table()) {
        ...
        if (s == "top-menu") {
            ...
            QString label = QString::fromLocal8Bit(s_label);
    ...
    for (const auto &[i, j] : config.as_table()) {
        ...
        std::string s = toml::get<std::string>(config[i]["type"]);
        if (s == "menu-item") {
            QString menuName = str_from_toml(config[i]["menu"]);
            QString cmdName = str_from_toml(config[i]["command"]);
            QString icon = str_from_toml(config[i]["icon"]);
            QString toolTip = str_from_toml(config[i]["tooltip"]);
            QString statusTip = str_from_toml(config[i]["statustip"]);
...
void MainWindow::readSettings()
{
    ...
    if (setting_toml.contains("general")) {
        ...
        settings_general_language = QString::fromStdString(toml::find<std::string>(table, "language"));
        settings_general_icon_theme = QString::fromStdString(toml::find<std::string>(table, "icon_theme"));
    }
