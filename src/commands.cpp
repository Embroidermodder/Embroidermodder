...
void add_to_selector(QComboBox* box, std::vector<std::string> list, EmbString type,
    int use_icon);

QWidget *get_widget(const char *key);
...
get_widget(const char *key)
{
    ...
    return (QWidget*)widget_list[index].widget;
}
...
show_widget(const char *key, int type)
{
    ...
    QWidget *widget = (QWidget*)widget_list[index].widget;
    widget->show();
}
...
{
    QLineEdit *lineEdit = qobject_cast<QLineEdit*>(get_widget(key));
    if (!lineEdit) {
        puts("Failed to find LineEdit widget.");
        return;
    }
    lineEdit->clear();
}
...
create_checkbox(QGroupBox* groupbox, int key)
{
    ...
    const char *label = settings_data[6*key + 2].c_str();
    const char *icon = settings_data[6*key + 3].c_str();
 ...
 */
void
add_to_selector(QComboBox* box, std::vector<std::string> list, EmbString type, int use_icon)
{
    int n = list.size() / 3;
    for (int i=0; i<n; i++) {
        if (!use_icon) {
            if (!strcmp(type, "real")) {
                box->addItem(list[3*i+0].c_str(), atof(list[3*i+1].c_str()));
                continue;
            }
            if (!strcmp(type, "int")) {
                box->addItem(list[3*i+0].c_str(), atoi(list[3*i+1].c_str()));
            }
            continue;
        }
        if (!strcmp(type, "string")) {
            box->addItem(create_icon(list[3*i+0].c_str()), list[3*i+1].c_str());
            continue;
        }
        if (!strcmp(type, "int")) {
            if (strlen(list[3*i+2].c_str()) > 0) {
                box->addItem(create_icon(list[3*i+0].c_str()), list[3*i+1].c_str(),
                    atoi(list[3*i+2].c_str()));
            }
            else {
                box->addItem(create_icon(list[3*i+0].c_str()), list[3*i+1].c_str());
            }
...
add_combobox(EmbString key, QComboBox *combobox)
{
    ...
    widget_list[n_widgets].widget = (void*)combobox;
    n_widgets++;
}
...
create_int_spinbox(QGroupBox* groupbox, int key)
{
    ...
    std::vector<std::string> settings_data = get_sl("settings_data");
    const char *name = settings_data[6*key + 0].c_str();
    const char *editor_data = settings_data[6*key + 5].c_str();
...
create_spinbox(QGroupBox* groupbox, int key)
{
    ...
    widget_list[n_widgets].widget = (void*)spinbox;
}
...
create_group_box(QWidget *parent, const char *key, const char *label)
{
    ...
    strcpy(widget_list[n_widgets].key, key);
    widget_list[n_widgets].widget = (void*)group_box;
    n_widgets++;
    return group_box;
}
...
create_label(QGroupBox *groupbox, const char *key, const char *text)
{
    ...
    strcpy(widget_list[n_widgets].key, key);
    widget_list[n_widgets].widget = (void*)label;
}
...
button_tip_of_the_day_clicked(int button)
{
    ...
    std::vector<std::string> tips = get_sl("tips");
    ...
    if (button == QWizard::CustomButton1) {
        ...
        labelTipOfTheDay->setText(tips[current].c_str());
...
obj_update_rubber(uint32_t obj_id, QPainter* painter)
{
    ...
    case RUBBER_ELLIPSE_MAJDIA_MINRAD: {
        ...
        // fixme: EmbReal ellipseWidth = obj_rubber_point(obj_id, "ELLIPSE_WIDTH").x;
...
/* . */
void
setBackgroundLogo(const char *fileName)
{
...
/* . */
void
setBackgroundTexture(const char *fileName)
{
...
/* . */
char *
qcolor_from_uint32_t(uint32_t color)
{
    return (char*)qPrintable(QColor(color).name());
}
...
MainWindow::MainWindow() : QMainWindow(0)
{
    ...
    for (int i=0; i<N_MENUS; i++) {
        menu[i] = new QMenu(translate(menu_list[i].c_str()), this);
    }

    std::vector<std::string> toolbar_list = get_sl("toolbar_list");
    for (int i=0; i<N_TOOLBARS; i++) {
        toolbar[i] = new QToolBar(translate(toolbar_list[i].c_str()), this);
    }
    ...
    std::vector<std::string> layer_list = get_sl("layer_list");
    ...
    std::vector<std::string> line_type_list = get_sl("line_type_list");
    ...
    std::vector<std::string> line_weight_list = get_sl("line_weight_list");
    ...
    std::vector<std::string> text_size_list = get_sl("text_size_list");
...
open_selected_files(char *files[])
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (mdiWin->loadFile(files[i])) {
            ...
            set_str(OPENSAVE_RECENT_DIRECTORY, qPrintable(QFileInfo(files[i]).absolutePath()));
...
/*! Rather than an iterator via ".find()" we need the integer index, so we
 * loop for a match here.
 */
int
get_index(std::vector<std::string> table, std::string key)
{
    int index = -1;
...
/*! . */
void
add_to_menu(int index, std::string menu_key)
{
    std::vector<std::string> menu_list = get_sl("menu_list");
    std::vector<std::string> menu_data = get_sl(menu_key);
    report_sl(menu_key);
    for (size_t i=0; i<menu_data.size(); i++) {
        const char *s = menu_data[i].c_str();
        if (s[0] == '-') {
            menu[index]->addSeparator();
        }
        else if (s[0] == '>') {
            int id = -1;
            for (size_t j=0; j<menu_list.size(); j++) {
                if (menu_list[j] == std::string(s+1)) {
                    id = j;
                    break;
                }
            }
...
read_settings(void)
{
    ...
    for (int i=0; i<N_SETTINGS; i++) {
        const char *label = settings_data[6*i+2].c_str();
        const char *value = settings_data[6*i+4].c_str();
        switch (settings_data[6*i+1].c_str()[0]) {
        case 'i': {
...
void
report_sl(std::string key)
{
    std::vector<std::string> table = get_sl(key);
...
{
    std::vector<std::string> toolbar_list = get_sl("toolbar_list");
    toolbar[id]->setObjectName(QString("toolbar") + toolbar_list[id].c_str());
...
PropertyEditor::PropertyEditor(QString iconDirectory, bool pickAddMode, QWidget* widgetToFocus, QWidget* parent) : QDockWidget(parent)
{
    ...
    for (int i=0; i < N_GROUPBOX; i++) {
        QGroupBox *gb = qobject_cast<QGroupBox*>(get_widget(group_box_list[i].key));
...
{
    QComboBox *comboBox = qobject_cast<QComboBox*>(get_widget(key));
    if (!comboBox) {
...
{
    strcpy(widget_list[n_widgets].key, key);
    widget_list[n_widgets].widget = (void*)lineedit;
    n_widgets++;
}
...
fieldEdited(QObject* fieldObj)
{
    ...
    foreach(QGraphicsItem* item, selectedItemList) {
        ...
        if (strncmp(label, "comboBox", strlen("comboBox")) == 0) {
            ObjectCore *core = tempObj->core;
            QComboBox *comboBox = qobject_cast<QComboBox*>(get_widget(label));
            if (!comboBox) {
                return;
            }
            ...
            QLineEdit* lineEdit = qobject_cast<QLineEdit*>(get_widget(label));
            if (!lineEdit) {
                puts("Failed to find lineEdit.");
                continue;
            }
...
{
    QPushButton* button = qobject_cast<QPushButton*>(get_widget("MdiBGLogoButton"));
    if (!button) {
        return;
    }
...
{
    QPushButton* button = qobject_cast<QPushButton*>(get_widget("MdiBGTextureButton"));
    if (!button) {
        return;
    }
...
combo_ruler_metric_index_changed(int32_t index)
{
    ...
    QComboBox *comboBox = qobject_cast<QComboBox*>(get_widget("Metric"));
    if (comboBox) {
...
{
    strcpy(prefix, STR(0));
    strcpy(curText, STR(0));
    promptInput->setText(STR(0));
    return script_null;
}
...
/* Exit the program: save prompt history, clear all memory allocated. */
ScriptValue
...
perpendicular_distance_command(ScriptEnv* context)
{
    ...
    QLineF norm = line.normalVector();
    EmbVector delta;
    delta.x = REAL(4) - REAL(0);
    delta.y = REAL(5) - REAL(1);
    norm.translate(delta.x, delta.y);
    QPointF iPoint;
    norm.intersects(line, &iPoint);
...
redo_command(ScriptEnv *context)
{
    ...
    if (dockUndoEdit->canRedo()) {
        ...
        dockUndoEdit->redo();
        //call(context, "set_prompt_prefix", prefix);
    }
    else {
...
tip_of_the_day_command(ScriptEnv * /* context */)
{
    ...
    std::vector<std::string> tips = get_sl("tips");
    if (get_int(GENERAL_CURRENT_TIP) >= tips.size()) {
        set_int(GENERAL_CURRENT_TIP, 0);
    }
    labelTipOfTheDay = new QLabel(tips[get_int(GENERAL_CURRENT_TIP)].c_str(),
        wizardTipOfTheDay);
