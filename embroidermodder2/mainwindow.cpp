...
QIcon MainWindow::load_icon(QString icon)
{
...
void MainWindow::createAllActions()
{
    ...
    for (int i=0; action_list[i].action>=0; i++) {
        QString icon = action_list[i].command;
        QString toolTip = tr(action_list[i].menu_label);
        QString statusTip = tr(action_list[i].description);
...
void MainWindow::add_to_toolbar(QToolBar *toolbar, std::string toolbar_description, int entries[])
{
...
void MainWindow::createPropertiesToolbar()
{
    ...
    colorSelector->addItem(load_icon("colorred"    ), tr("Red"),     qRgb(255,  0,  0));
    colorSelector->addItem(load_icon("coloryellow" ), tr("Yellow"),  qRgb(255,255,  0));
    colorSelector->addItem(load_icon("colorgreen"  ), tr("Green"),   qRgb(  0,255,  0));
    colorSelector->addItem(load_icon("colorcyan"   ), tr("Cyan"),    qRgb(  0,255,255));
    colorSelector->addItem(load_icon("colorblue"   ), tr("Blue"),    qRgb(  0,  0,255));
    colorSelector->addItem(load_icon("colormagenta"), tr("Magenta"), qRgb(255,  0,255));
    colorSelector->addItem(load_icon("colorwhite"  ), tr("White"),   qRgb(255,255,255));
    colorSelector->addItem(load_icon("colorother"  ), tr("Other..."));
...
void
MainWindow::check_load_file(std::string path)
{
    QString appDir = qApp->applicationDirPath();
    QFileInfo check(appDir + QString::fromLocal8Bit(path));
    if (!check.exists()) {
        QMessageBox::critical(this, tr("Path Error"), tr("Cannot locate: ")
...
MainWindow::MainWindow() : QMainWindow(0)
{
    ...
    QString language = settings_general_language;
    ...
    QString s = appDir + "/icons/" + settings_general_icon_theme;
...
{
    debug_message("MainWindow::findMdiWindow(" + fileName.toStdString() + ")");
