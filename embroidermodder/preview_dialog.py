#!/usr/bin/env python3

r"""
    Embroidermodder 2.

    -----

    Copyright 2013-2022 The Embroidermodder Team
    Embroidermodder 2 is Open Source Software.
    See LICENSE for licensing terms.

    -----

"""

from embroidermodder.image_widget import Image_Widget
from embroidermodder.utility import debug_message

class Preview_Dialog():
    """
    .
    """
    def __init__(self, parent, caption, dir, filter):
        " . "
        debug_message("PreviewDialog Constructor")

        #TODO: get actual thumbnail image from file, lets also use a size of 128x128 for now...
        #TODO: make thumbnail size adjustable thru self.settings dialog
        self.imgWidget = Image_Widget("icons/default/nopreview.png")

        lay = layout()
        if lay:
            columns = lay.columnCount()
            rows = lay.rowCount()
            lay.addWidget(self.imgWidget, 0, columns, rows, 1)

        self.modal = 1
        self.option = "QFileDialog_DontUseNativeDialog"
        self.view_mode = "QFileDialog_Detail"
        self.file_mode = "QFileDialog_ExistingFiles"

        #TODO: connect the currentChanged signal to update the preview imgWidget.

    def toPolyline(self, pattern, objPos, objPath, layer, color, lineType,  lineWeight):
        debug_message(".")