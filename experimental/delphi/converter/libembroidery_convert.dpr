...
var
  i : integer;
  formatList, current : PEmbFormatList;
...
begin
  ...
  current  := formatList;
    ...
    begin
      embFormat_info( current.extension, ext, description, stateReader, stateWriter, formatType ); 
      writeln(format('|  %-4s  |   %s   |   %s   |  %-49s |', [ current^.extension, stateReader, stateWriter, description]) );
