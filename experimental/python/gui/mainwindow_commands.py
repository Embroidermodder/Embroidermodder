...
def updateAllViewScrollBars(self, val):
    ...
    for i in range(0, windowList.count()):  # for(int i = 0; i < windowList.count(); ++i)
        mdiWin = windowList.at(i)  # MdiWindow* mdiWin = qobject_cast<MdiWindow*>(windowList.at(i));
        if mdiWin:
...
def zoomSelected(self):
    ...
    if gview and stack:
        cmd = UndoableNavCommand("ZoomSelected", gview, 0)  # UndoableNavCommand*
        stack.push(cmd)
...
def zoomExtents(self):
    ...
    stack = gview.getUndoStack()  # QUndoStack*
    if gview and stack:
        cmd = UndoableNavCommand("ZoomExtents", gview, 0)  # UndoableNavCommand*
        stack.push(cmd)
...
def panLeft(self):
    ...
    stack = gview.getUndoStack()  # QUndoStack*
    if gview and stack:
        cmd = UndoableNavCommand("PanLeft", gview, 0)  # UndoableNavCommand*
        stack.push(cmd)
...
def panUp(self):
    ...
    stack = gview.getUndoStack()  # QUndoStack*
    if gview and stack:
        cmd = UndoableNavCommand("PanUp", gview, 0)  # UndoableNavCommand*
        stack.push(cmd)
...
def panDown(self):
    ...
    stack = gview.getUndoStack()  # QUndoStack*
    if gview and stack:
        cmd = UndoableNavCommand("PanDown", gview, 0)  # UndoableNavCommand*
        stack.push(cmd)
...
def nativeAddPolygon(self, startX, startY, p, rubberMode):
    ...
    stack = gview.getUndoStack()  # QUndoStack*
    if gview and gscene and stack:
        obj = PolygonObject(startX, startY, p, self.getCurrentColor())  # PolygonObject*
        obj.setObjectRubberMode(rubberMode)
        if rubberMode:
            gview.addToRubberRoom(obj)
            gscene.addItem(obj)
...
def nativeAddPolyline(self, startX, startY, p, rubberMode):
    ...
    stack = gview.getUndoStack()  # QUndoStack*
    if gview and gscene and stack:
        obj = PolylineObject(startX, startY, p, self.getCurrentColor())  # PolylineObject*
        obj.setObjectRubberMode(rubberMode)
        if rubberMode:
            gview.addToRubberRoom(obj)
            gscene.addItem(obj)
...
def nativeAddDimLeader(self, x1, y1, x2, y2, rot, rubberMode):
    ...
    stack = gview.getUndoStack()  # QUndoStack*
    if gview and gscene and stack:
        obj = DimLeaderObject(x1, -y1, x2, -y2, self.getCurrentColor())  # DimLeaderObject*
        obj.setRotation(-rot)
        obj.setObjectRubberMode(rubberMode)
        if rubberMode:
            gview.addToRubberRoom(obj)
